...
var shopCar = (function () {
    ...
    function allNumPrice() {
        ...
        $('.checkSingle').each(function (i, val) {
            if (val.checked) {
                ...
                var $sinN = parseInt($('.sin-num').eq($(this).index('.checkSingle')).html());
                // var $sinP = parseFloat($('.sin-price').eq(i).html()) * $sinN;
                var $sinP = parseFloat($('.sin-price').eq($(this).index('.checkSingle')).html()) * $sinN;
                $allNum += $sinN;
                $allPrice += $sinP;
    ...
    function diaoYong(){
        allSelect();sinSelect();addClick();reduceClick();delNode();delAll1();delAll2();delAll3();
    }
    return {
        xianNone:xianNone,
        diaoYong:diaoYong
        ...
        $(window).on('load', function () {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    shopCar.diaoYong();//数据渲染完以后,调用上面的函数
                    diaoYong2();
                },
                error: function () {
                    shopCar.xianNone();//数据渲染完以后,调用上面的函数
                }
            })
        })
    }
...
var operation = (function () {
    function winHis(){
        $('.car-head .car-left').on('click',function(){
            window.history.back();
        })
    }
    //商品点击添加
    ...
    function diaoYong3(){
        addShop();winData();handCookie();winHis()
    }
    return {
        diaoYong3:diaoYong3
    }
